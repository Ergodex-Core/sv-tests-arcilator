...
TEST_LIST_FILE="${TEST_LIST_FILE:-"${WS_ROOT}/tempnotes/NOTES/m3_scheduler_acceptance_tests.txt"}"

OUT_ARC="${OUT_ARC:-out_arcilator_m3_scheduler_accept}"
OUT_VER="${OUT_VER:-out_Verilator_m3_scheduler_accept}"

OVERRIDE_TEST_TIMEOUTS="${OVERRIDE_TEST_TIMEOUTS:-1800}"
FAIL_ON_GAPS="${FAIL_ON_GAPS:-1}"

# Ensure arcilator advances time in a consistent unit (runner uses timescale=1ns).
export ARCILATOR_SIM_DT_FS="${ARCILATOR_SIM_DT_FS:-1000000}"
...
echo "[run] arcilator_top → ${OUT_ARC} (ARCILATOR_SIM_DT_FS=${ARCILATOR_SIM_DT_FS})"
cd "${SVTESTS_ROOT}"
RUNNERS=arcilator_top ./one.sh report OUT_DIR="${OUT_ARC}" TESTS="${TESTS_STR}" -B -j"$(nproc)"

ensure_verilator "${OUT_VER}"
echo "[run] Verilator → ${OUT_VER} (OVERRIDE_TEST_TIMEOUTS=${OVERRIDE_TEST_TIMEOUTS})"
...
out_arc = os.environ.get("OUT_ARC", "out_arcilator_m3_scheduler_accept")
out_ver = os.environ.get("OUT_VER", "out_Verilator_m3_scheduler_accept")
fail_on_gaps = os.environ.get("FAIL_ON_GAPS", "1").strip() not in ("0", "false", "no", "off")
