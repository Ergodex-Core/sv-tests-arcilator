...
# Run arcilator in top-executed mode (`RUNNERS=arcilator_top`) on the full
# UVM-tagged baseline list derived from the published dashboard snapshot
# (UVM138).

SVTESTS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DATA_DIR="${DATA_DIR:-"${SVTESTS_DIR}/tools/data"}"
UVM138_LIST="${UVM138_LIST:-"${DATA_DIR}/uvm138_tests.txt"}"

OUT_DIR="${OUT_DIR:-"${SVTESTS_DIR}/out_arcilator_top_uvm138_all"}"
...
if [[ ! -f "${UVM138_LIST}" ]]; then
  echo "[error] missing UVM138_LIST=${UVM138_LIST}" >&2
  exit 2
fi
...
TESTS_DIR="${SVTESTS_DIR}/tests" UVM138_LIST="${UVM138_LIST}" python3 - >"${tmp_tests_file}" <<'PY'
import os
import sys
from pathlib import Path

tests_root = Path(os.environ["TESTS_DIR"])
uvm138_list = Path(os.environ["UVM138_LIST"])

selected = []
missing = 0
for raw in uvm138_list.read_text(encoding="utf-8", errors="ignore").splitlines():
    rel = raw.strip()
...
sys.stderr.write(f"[select] uvm138 (list={uvm138_list}) mapped={len(selected)} missing={missing}\n")
for rel in selected:
    print(rel)
PY
