...
class arcilator(BaseRunner):
    ...
    UVM notes:
      - By default, UVM-tagged tests run the first module (DUT) to avoid
        selecting class-heavy `module top` benches which arcilator cannot
        truly execute.
      - Set `ARCILATOR_UVM_TOP_MODE=top` to prefer `module top` *when it
        contains concurrent assertions* (`assert property` / `assume property`
        / `cover property`).
      - Set `ARCILATOR_UVM_TOP_STRICT=1` to force `module top` whenever it
        exists (even for class-heavy benches).
    ...
    def _pick_top_module(self, params):
        ...
        if "uvm" in tags:
            ...
            #
            # Note: Many UVM tests have a class-based `module top` which
            # arcilator cannot truly execute. By default we only select `top`
            # when it contains concurrent assertions (`assert property`); to
            # force `top` always, also set ARCILATOR_UVM_TOP_STRICT=1.
