...
# UVM-tagged tests that support `:type: simulation` (i.e. the ones a Verilator
# run would execute in simulation mode).

SVTESTS_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

OUT_DIR="${OUT_DIR:-"${SVTESTS_DIR}/out_arcilator_top_uvm_verilator_sim"}"
