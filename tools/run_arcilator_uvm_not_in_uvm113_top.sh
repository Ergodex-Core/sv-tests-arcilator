...
# Run arcilator in top-executed mode (`RUNNERS=arcilator_top`) on all UVM-tagged
# tests that are NOT part of the `tools/data/uvm113_tests.txt` list.
...
OUT_DIR="${OUT_DIR:-"${SVTESTS_DIR}/out_arcilator_top_uvm_not_in_uvm113"}"
...
selected = sorted(set(all_uvm) - uvm113)
sys.stderr.write(f"[select] uvm total={len(set(all_uvm))} minus uvm113={len(uvm113)} => {len(selected)} tests\n")
for rel in selected:
    print(rel)
PY
