...
TEST_LIST_FILE="${TEST_LIST_FILE:-"${WS_ROOT}/tempnotes/NOTES/top_exec_acceptance_tests.txt"}"

OUT_ARC="${OUT_ARC:-out_arcilator_top_exec_accept}"
OUT_VER="${OUT_VER:-out_Verilator_top_exec_accept}"
...
# Ensure generated tests exist if the list references them.
if printf '%s\n' "${TESTS_ARR[@]}" | grep -q '^generated/'; then
  if [[ ! -d "${SVTESTS_ROOT}/tests/generated" ]]; then
    echo "[info] generated tests missing; running template generator"
...
echo "[run] arcilator_top → ${OUT_ARC}"
cd "${SVTESTS_ROOT}"
...
out_arc = os.environ.get("OUT_ARC", "out_arcilator_top_exec_accept")
out_ver = os.environ.get("OUT_VER", "out_Verilator_top_exec_accept")
