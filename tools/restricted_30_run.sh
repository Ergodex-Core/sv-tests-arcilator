#!/usr/bin/env bash
# Re-run a fixed set of ~30 tests, capture artifacts per test, and (when
# possible) emit a VCD. For modules without ports we synthesize a simple input/
# output port pair so the harness can drive/observe a waveform.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OUT_ROOT="${OUT_ROOT:-"$ROOT/out/artifacts/restricted_30"}"
TMP_ROOT="${TMP_ROOT:-"$ROOT/out/tmp_restricted_30"}"
