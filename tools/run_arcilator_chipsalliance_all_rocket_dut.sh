...
# Run arcilator in DUT mode (`RUNNERS=arcilator`) on every test named in
# `chipsalliance.csv` (unique TestName values that can be mapped to local
# sv-tests sources).
#
# Note: this disables the UVM M0 semantic gate for the arcilator runner (DUT
# mode does not execute class-based UVM testbenches).
...
OUT_DIR="${OUT_DIR:-"${SVTESTS_DIR}/out_arcilator_dut_chipsalliance_all"}"
