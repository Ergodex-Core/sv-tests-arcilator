...
# Run arcilator in top-executed mode (`RUNNERS=arcilator_top`) on the
# simulation-capable subset of the UVM113 list.
...
OUT_DIR="${OUT_DIR:-"${SVTESTS_DIR}/out_arcilator_top_uvm113_verilator_sim"}"
...
sys.stderr.write(f"[select] uvm113 (list={uvm113_list}) missing={missing} type=simulation: {len(selected)} tests\n")
for rel in selected:
    print(rel)
PY
